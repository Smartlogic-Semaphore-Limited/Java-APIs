...
/**
 * Example class showing how to use the Cloud API and the Classification API in
 * the same context
 * 
...
//		classificationClient.setProxyPort(8888);
			...
			for (Entry<String, Collection<ClassificationScore>> entry : result.getAllClassifications().entrySet()) {
				...
				for (ClassificationScore classificationScore : entry.getValue()) {
					System.out.println(String.format(" %s %s %s %f", classificationScore.getRulebaseClass(),
							classificationScore.getId(), classificationScore.getName(),
