...
public class Concept extends IdentifiableObject {
	...
	/**
	 * Adds the preferred label - if there is already a preferred label of this language an
	 * exception is thrown.
	 *
	 * @param label - the preferred label in the requested language.
	 * @throws ModelException - thrown if there is already a label of this language
	 *                        for the object
	 */
	 ...
	 * other in this language. It does not matter whether there already was a label
	 * of this language present.
	 *
	...
	/**
	 * updateLabel Delete the current label in this language and replace it with the
	 * supplied one If there was no label in this language, then throw an exception
	 *
	 * @param label - the label to be updated on this object.
	 * @throws ModelException - thrown if there is no label in the supplied language
	 *                        to update
	 */
	...
	/**
	 * Delete the supplied preferred label from this concept scheme. If the label is not
	 * present on the concept scheme then an exception will be thrown.
	 *
	...
	public Label getPrefLabelForLanguage(String langCode) throws ModelException {
		...
		pSparql.setLiteral("labelLanguage", Strings.isNullOrEmpty(langCode) ?
				model.createLiteral("") : model.createLiteral(langCode));
	...
	private void addPrefLabelPostCheck(Label label) throws ModelException {
		String labelUri = resource.getURI() + Utils.encodeStringForURI(label.getValue()) +
	...
	private void addAltLabelPostCheck(Label altLabel) throws ModelException {
		Resource altLabelRes = model.createResource(
				resource.getURI() + Utils.encodeStringForURI(altLabel.getValue()) +
						(Strings.isNullOrEmpty(altLabel.getLanguageCode()) ? "" : "_" + altLabel.getLanguageCode()));
		resource.addProperty(SKOSXL.altLabel, altLabelRes);
	...
	private void checkLabelExistsInLanguage(Label label) throws ModelException {
		if (!prefLabelExistsInLanguage(label))
			throw new ModelException("Attempting to delete label for '%s'. This already has no label in language '%s'",
					resource.getURI(), label.getLanguageCode());
	}

	private void checkPrefLabelDoesntExistInLanguage(Label label) throws ModelException {
		if (prefLabelExistsInLanguage(label))
			throw new ModelException("Attempting to create label for '%s'. This already has a label in language '%s'",
					resource.getURI(), label.getLanguageCode());
	}
	...
	private boolean checkAltLabelExists(Label label) {
		...
		parameterizedSparql.setCommandText(sparql);
		parameterizedSparql.setParam("conceptUri", resource);
		parameterizedSparql.setParam("labelLanguage", Strings.isNullOrEmpty(label.getLanguageCode()) ?
				model.createLiteral("") :
