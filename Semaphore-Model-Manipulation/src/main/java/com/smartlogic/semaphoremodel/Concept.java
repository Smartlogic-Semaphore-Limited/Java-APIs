...
public class Concept extends ConceptObject {
	...
	public void addRelationship(ConceptToConceptRelationshipType relationshipType, Concept concept2) {
		...
		if (relationshipType.getInverseProperty() == null) {
			...
			concept2.getResource().addProperty(relationshipType.getInverseProperty(), this.getResource()); // Assymmetric
																											// property
		}
	}

	/**
	 * addLabel Adds the label - if there is already a label of this language an
	 * exception is thrown.
	 * 
	...
	/**
	 * setLabel After this operation, the Concept Scheme will have this label and no
	...
	/**
	 * Delete the supplied label from this concept scheme. If the label is not
	...
	private void addLabel(Property property, Label label) {
		Resource labelResource = model.createResource(this.getResource().toString() + "_" + property.getLocalName()
				+ "_" + label.getLanguageCode() + "_" + getBase64(label.getValue()));
