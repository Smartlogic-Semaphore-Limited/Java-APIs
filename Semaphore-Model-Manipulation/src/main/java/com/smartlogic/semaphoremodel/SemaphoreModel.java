...
/**
 * A wrapper class around a Jena Model object that provides easy to use methods to manipulate the
 * model.
 */
public class SemaphoreModel {
   ...
   *
   * @param model
   *          the model from which to build the URI
   * @param conceptURIResource
   *          the concept URI for which to associate the Label object.
   * @param relationshipProperty
   *          the property tye of the label.
   * @param label
   *          the label literal string
   * @return the URI object for the label.
   * @throws ModelException
   *           ModelException
   */
  static URI getLabelURI(Model model, Resource conceptURIResource, Property relationshipProperty,
      Label label) throws ModelException {
    URI labelURI = label.getURI();
    if (labelURI != null) {
      if (uriInUse(model, labelURI)) {
        throw new ModelException(
            "Attempting to create label with URI - '%s'. This URI is already in use.",
            labelURI.toString());
      }
    } else {
      int counter = 0;
      String uriString = conceptURIResource.getURI() +
          "_" +
          relationshipProperty.getLocalName() +
          "_" +
          label.getLanguageCode();
      try {
        labelURI = new URI(uriString);
        while (uriInUse(model, labelURI)) {
          uriString = conceptURIResource.getURI() +
              "_" +
              relationshipProperty.getLocalName() +
              "_" +
              label.getLanguageCode() +
   ...
   *
   * @param model
   *          the Jena Model object.
   * @param uri
   *          the URI of the object to check.
   ...
   *
   * @param model
   *          the Jena model to use to construct the Resource object.
   * @param uri
   *          the URI for the Resource object.
   ...
   *          the Jena model to use to construct the Resource object.
   * @param prefLabel
   *          the label object from which to construct the Jena Literal object.
  ...
  /**
   * Builds a new SemaphoreModel object that wraps the specified Jena model and sets up common
   * namespace prefixes.
   *
   * @param model
   *          the Jena model that contains the triples.
   */
  ...
  /**
   * Builds a new SemaphoreModel and populates the model with triples from the specified file. Sets
   * up common namespace prefixes.
   *
   * @param inputFile
   *          the triples file from which to populate the SemaphoreModel.
   */
  public SemaphoreModel(File inputFile) {
    ...
    logger.info(String.format("Data read in %#.3f seconds",
        0.001 * (readDate.getTime() - startDate.getTime())));

    setPrefixes();
  }

  /**
   * Builds a new SemaphoreModel and populates the model with triples from the specified files. Sets
   * up common namespace prefixes. All files are added to the same model (not separate named
   * graphs).
   *
   * @param inputFiles
   *          the set of triples files from which to populate the SemaphoreModel.
   */
  ...
  /**
   * Sets common namespace prefixes: sem, xsd, skos, skosxl, rdf, rdfs, owl and sioc in the Jena
   * model. Serialized models will use these namespace prefixes for brevity.
   */
  ...
  /**
   * Load the specified file into the current SemaphoreModel object (Jena Model). All triples are
   * loaded into the wrapped model object, not as separate named graphs.
   *
   * @param inputFile
   *          the input from which to load the triples. Must be valid triple file (RDF, TTL, NT)
   */
  ...
  /**
   * Construct a new SemaphoreModel object backed by TDB database in specified folder with the
   * specified graph name, optionally resetting the model when it loads.
   *
   * @param tdbDirectory
   *          The directory on disk in which to store the model database
   * @param graphName
   *          the name of graph (uri syntax)
   * @param resetTDB
   *          whether to reset (clear) the model.
   * @throws IOException
   *           IOException
   */
  ...
  /**
   * Write the model to the specified file. If the file name has the .n3 extention, write the model
   * as n3 format. If the file name has the extension .rdf or .xml, write the model as RDF/XML
   * format. Otherwise, write the model in TTL (turtle) format.
   *
   * @param file
   *          the File object into which to write the file. Will be created if not present, cleared
   *          otherwise.
  ...
  /**
   * Create and add a Concept to the model. The concept UUID will be randomly generated.
   *
   * @param uri
   *          the new concept's URI.
   * @param prefLabel
   *          the new concept's preferred label.
  ...
  /**
   * Creates and adds a new concept with the specified concept class and preferred label. The
   * concept UUID will be randomly generated.
   *
   * @param uri
   *          the new concept's URI
   * @param prefLabel
   *          the new concept's preferred label.
   * @param conceptClass
   *          the new concept's concept class
   * @return the new Concept object.
   * @throws ModelException
   *           ModelException
   */
  public Concept createConcept(URI uri, Label prefLabel, ConceptClass conceptClass)
      throws ModelException {
    return createConcept(uri, prefLabel, new ConceptClass[] { conceptClass });
  }

  /**
   * Create and add a concept to the model with the specified set of concept classes. The concept
   ...
   *          an array of concept classes for the concept
   * @return the new Concept object.
   * @throws ModelException
   *           ModelException
   */
  public Concept createConcept(URI uri, Label prefLabel, ConceptClass[] conceptClasses)
      throws ModelException {
    return createConcept(uri, prefLabel, conceptClasses, null);
  }

  /**
   * Create and add a concept to the model with the specified set of concept classes, specified
   * preferred label and UUID.
   *
   * @param uri
   *          the new concept's URI
   * @param prefLabel
   ...
   *          an array of concept classes for the concept
   * @param uuid
   *          the UUID for the concept.
   * @return the new Concept object.
   * @throws ModelException
   *           ModelException
   */
  public Concept createConcept(URI uri, Label prefLabel, ConceptClass[] conceptClasses, UUID uuid)
      throws ModelException {
  ...
  /**
   * Create and add a concept to the model with the specified preferred label and UUID. The concept
   * class will be set to skos:Concept.
   *
   * @param uri
   *          the new concept's URI
   * @param prefLabel
  ...
  /**
   * Create and add a concept to the model with the specified set of concept classes (as Jena
   * Resource objects), specified preferred label and UUID.
   *
   * @param uri
   *          the new concept's URI.
   * @param prefLabel
   ...
   *          an array of Jena Resource objects representing classes
   * @param uuid
   *          the new concept's UUID
   * @return the new Concept object.
   * @throws ModelException
   *           ModelException
   */
  private Concept createConcept(URI uri, Label prefLabel, Resource[] classResources, UUID uuid)
      throws ModelException {
    Resource conceptURIResource = resourceFromURI(model, uri);

    if (resourceInUse(conceptURIResource)) {
      throw new ModelException(
          "Attempting to create concept with URI - '%s'. This URI is already in use.",
          uri.toString());
    }
    ...
    conceptURIResource.addLiteral(SEM.guid,
        (uuid == null ? Utils.generateGuid(uri.toString()) : uuid.toString()));
    ...
    prefLabel.setResource(labelURIResource);
  ...
  /**
   * Returns true if the specified Resource object is used in any triples. Used to identify existing
   * resources when attempting to add new ones.
   *
   * @param resource
   *          the Jena Resource object (URI)
   ...
   *
   * @param uri
   *          the new concept scheme's URI.
   * @param label
   *          the new concept scheme's label.
   * @param uuid
   *          the new concept scheme's UUID.
  ...
  public ConceptScheme createConceptScheme(URI uri, Label label, UUID uuid) throws ModelException {
    ...
    if (resourceInUse(conceptSchemeURIResource)) {
      throw new ModelException(String.format(
          "Attempting to create concept scheme with URI - '%s'. This URI is already in use.",
          uri.toString()));
    }

    conceptSchemeURIResource.addProperty(RDF.type, SKOS.ConceptScheme);

    conceptSchemeURIResource.addLiteral(SEM.guid,
        (uuid == null ? Utils.generateGuid(uri.toString()) : uuid.toString()));
  ...
  /**
   * Create and add a new concept class to the model with the specified URI and label. This concept
   * class with be a sub-class of skos:Concept.
   *
   * @param uri
   *          the new concept class' URI
   * @param label
   *          the new concept class' label
  ...
  /**
   * Create and add a new concept class to the model with the specified URI and label. This concept
   * class with be a sub-class of specified concept class.
   *
   * @param uri
   *          the new concept class' URI
   * @param label
   *          the new concept class' label
   * @param parentURI
   *          the URI of the parent concept class.
   * @return the new ConceptClass object.
   * @throws ModelException
   *           ModelException
   */
  public ConceptClass createConceptClass(URI uri, Label label, URI parentURI)
      throws ModelException {
  ...
  /**
   * Create and add a new concept class to the model with the specified URI and label. This concept
   * class with be a sub-class of specified concept class (as Jena Resource).
   *
   * @param uri
   *          the new concept class' URI
   * @param label
   ...
   *          the URI of the parent concept class as Jena Resource.
   * @return the new ConceptClass object.
   * @throws ModelException
   *           ModelException
   */
  public ConceptClass createConceptClass(URI uri, Label label, Resource parentResource)
      throws ModelException {
    Resource conceptClassURIResource = resourceFromURI(model, uri);

    if (resourceInUse(conceptClassURIResource)) {
      throw new ModelException(String.format(
          "Attempting to create concept class with URI - '%s'. This URI is already in use.",
          uri.toString()));
    }
   ...
   * @return the Concept object.
   */
  public ConceptClass getConceptClass(URI uri) {
    Resource conceptClassResource = resourceFromURI(model, uri);
    String sparql = "ASK WHERE { ?conceptClassURI rdfs:subClassOf* skos:Concept . }";
    ParameterizedSparqlString findConceptSparql = new ParameterizedSparqlString(model);
    findConceptSparql.setCommandText(sparql);
    findConceptSparql.setParam("conceptClassURI", conceptClassResource);

    Query query = QueryFactory.create(findConceptSparql.asQuery());

    QueryExecution qexec = QueryExecutionFactory.create(query, model);
    return qexec.execAsk() ? new ConceptClass(model, conceptClassResource) : null;
  }
   ...
   *
   * @param uri
   *          the URI from which to build the Resource object.
   * @return the Jena Resource object.
   */
  ...
  /**
   * Construct a ConceptClass object from the query solution. Required parameter names: cc_uri and
   * cc_label_lit.
   *
   * @param qs
   *          the query solution from which to build the ConceptClass.
   * @return the ConceptClass object.
   * @throws ModelException
   *           ModelException
   */
  private ConceptClass constructConceptClassFromQuerySolution(QuerySolution qs)
      throws ModelException {
    if (null == qs) {
      return null;
    }
  ...
  private Resource createMetadataTypeResource(URI uri, Label label, Resource range)
      throws ModelException {

    Resource metadataTypeURIResource = resourceFromURI(model, uri);
    if (resourceInUse(metadataTypeURIResource)) {
      throw new ModelException(
          "Attempting to create metadata type with URI - '%s'. This URI is already in use.",
          uri.toString());
    }
  ...
  public CalendarMetadataType createCalendarMetadataType(URI uri, Label label)
      throws ModelException {
  ...
  public MetadataType getMetadataTypeIfExists(URI uri) throws ModelException {
    String sparql =
        "SELECT ?metadataTypeURI ?range WHERE { VALUES ?metadataTypeURI { ?suppliedURI } . ?metadataTypeURI a owl:DatatypeProperty .  ?metadataTypeURI rdfs:range ?range . }";
    ParameterizedSparqlString findConceptSchemeSparql = new ParameterizedSparqlString(model);
    findConceptSchemeSparql.setCommandText(sparql);
    ...
    while (resultSet.hasNext()) {
      ...
      if (XSD.xstring.equals(range)) {
        ...
        metadataType =
            new CalendarMetadataType(model, querySolution.getResource("?metadataTypeURI"));
      } else if (XSD.xboolean.equals(range)) {
        metadataType =
            new BooleanMetadataType(model, querySolution.getResource("?metadataTypeURI"));
      } else if (XSD.integer.equals(range)) {
        metadataType =
            new IntegerMetadataType(model, querySolution.getResource("?metadataTypeURI"));
      } else {
        throw new ModelException(
            "Unable to determine range for metadata type with URI: '%s' - '%s'", uri, range);
      }
    }
    return metadataType;
  }
  ...
  public HasBroaderRelationshipType createHasBroaderRelationshipType(URI uri, Label label,
      URI inverseURI, Label inverseLabel) throws ModelException {
    return createHasBroaderRelationshipType(uri, label, inverseURI, inverseLabel, null, null);
  }

  public HasBroaderRelationshipType createHasBroaderRelationshipType(URI uri, Label label,
      URI inverseURI, Label inverseLabel, ConceptClass domain, ConceptClass range)
      throws ModelException {
    Resource relationshipTypeResource =
        createConceptToConceptRelationshipTypeResource(uri, label, SKOS.broader, domain, range);
    Resource inverseRelationshipTypeResource = null;
    if (inverseURI != null) {
      inverseRelationshipTypeResource = createConceptToConceptRelationshipTypeResource(inverseURI,
          inverseLabel, SKOS.narrower, range, domain);
    ...
    return new HasBroaderRelationshipType(model, relationshipTypeResource,
        inverseRelationshipTypeResource);
  }

  public HasNarrowerRelationshipType createHasNarrowerRelationshipType(URI uri, Label label,
      URI inverseURI, Label inverseLabel) throws ModelException {
  ...
  public HasNarrowerRelationshipType createHasNarrowerRelationshipType(URI uri, Label label,
      ...
      throws ModelException {
    Resource relationshipTypeResource =
        createConceptToConceptRelationshipTypeResource(uri, label, SKOS.narrower, domain, range);
    Resource inverseRelationshipTypeResource = null;
    if (inverseURI != null) {
      inverseRelationshipTypeResource = createConceptToConceptRelationshipTypeResource(inverseURI,
          inverseLabel, SKOS.broader, range, domain); // Inverse has domain and range reversed
      relationshipTypeResource.addProperty(OWL.inverseOf, inverseRelationshipTypeResource);
      inverseRelationshipTypeResource.addProperty(OWL.inverseOf, relationshipTypeResource);
    }
    return new HasNarrowerRelationshipType(model, relationshipTypeResource,
        inverseRelationshipTypeResource);
  }

  public AssocativeRelationshipType createAssociativeRelationshipType(URI uri, Label label,
      URI inverseURI, Label inverseLabel) throws ModelException {
  ...
  public AssocativeRelationshipType createAssociativeRelationshipType(URI uri, Label label,
      ...
      throws ModelException {
    Resource relationshipTypeResource =
        createConceptToConceptRelationshipTypeResource(uri, label, SKOS.related, domain, range);
    Resource inverseRelationshipTypeResource = null;
    if (inverseURI == null) {
      relationshipTypeResource.addProperty(RDF.type, OWL.SymmetricProperty);
    } else {
      inverseRelationshipTypeResource = createConceptToConceptRelationshipTypeResource(inverseURI,
          inverseLabel, SKOS.related, range, domain); // Inverse has domain and range reversed
      relationshipTypeResource.addProperty(OWL.inverseOf, inverseRelationshipTypeResource);
      inverseRelationshipTypeResource.addProperty(OWL.inverseOf, relationshipTypeResource);
    }
    return new AssocativeRelationshipType(model, relationshipTypeResource,
        inverseRelationshipTypeResource);
  }

  public HasEquivalentRelationshipType createHasEquivalentRelationshipType(URI uri, Label label)
      throws ModelException {
    Resource relationshipTypeResource =
        createConceptToLabelRelationshipTypeResource(uri, label, SKOSXL.altLabel, null);
    return new HasEquivalentRelationshipType(model, relationshipTypeResource);
  }

  public HasEquivalentRelationshipType createHasEquivalentRelationshipType(URI uri, Label label,
      Resource domain) throws ModelException {
    Resource relationshipTypeResource =
        createConceptToLabelRelationshipTypeResource(uri, label, SKOSXL.altLabel, null);
    return new HasEquivalentRelationshipType(model, relationshipTypeResource);
  }

  private Resource createRelationshipTypeResource(URI uri, Label label, Property parentProperty,
      ConceptClass domain) throws ModelException {
    Resource relationshipTypeURIResource = resourceFromURI(model, uri);
    if (resourceInUse(relationshipTypeURIResource)) {
      throw new ModelException(
          "Attempting to create relationship type with URI - '%s'. This URI is already in use.",
          uri.toString());
    }
    ...
    relationshipTypeURIResource.addProperty(RDFS.domain,
        domain == null ? SKOS.Concept : domain.getResource());
    relationshipTypeURIResource.addProperty(RDFS.subPropertyOf, parentProperty);
    return relationshipTypeURIResource;

  }

  private Resource createConceptToConceptRelationshipTypeResource(URI uri, Label label,
      Property parentProperty, ConceptClass domain, ConceptClass range) throws ModelException {
    Resource relationshipTypeURIResource =
        createRelationshipTypeResource(uri, label, parentProperty, domain);
    relationshipTypeURIResource.addProperty(RDFS.range,
        range == null ? SKOS.Concept : range.getResource());
    return relationshipTypeURIResource;
  }

  private Resource createConceptToLabelRelationshipTypeResource(URI uri, Label label,
      Property parentProperty, ConceptClass domain) throws ModelException {
    Resource relationshipTypeURIResource =
        createRelationshipTypeResource(uri, label, parentProperty, domain);
  ...
  public RelationshipType getRelationshipType(URI uri) throws ModelException {
    ...
    while (resultSet.hasNext()) {
      ...
      Resource inverseRelationshipTypeResource =
          querySolution.getResource("?inverseRelationshipType");
      ...
      if ("broader".equals(relationshipBaseTypeResource.getLocalName())) {
        relationshipType = new HasBroaderRelationshipType(model, relationshipTypeResource,
            inverseRelationshipTypeResource);
      } else if ("narrower".equals(relationshipBaseTypeResource.getLocalName())) {
        relationshipType = new HasNarrowerRelationshipType(model, relationshipTypeResource,
            inverseRelationshipTypeResource);
      } else if ("related".equals(relationshipBaseTypeResource.getLocalName())) {
        relationshipType = new AssocativeRelationshipType(model, relationshipTypeResource,
            inverseRelationshipTypeResource);
        ...
        throw new ModelException("Unrecognized base type: '%s' for relationship '%s'",
            relationshipBaseTypeResource.getLocalName(), uri.toString());
      }
    }
    return relationshipType;
  }

  private <T extends RelationshipType> T getRelationshipType(URI uri, Resource baseTypeResource,
      Class<T> clazz) {
    String sparql = "select ?relationshipTypeUri ?inverseRelationshipType where {" +
        "  ?relationshipTypeUri rdfs:subPropertyOf* ?relationshipBaseType ." +
        "  VALUES ?relationshipTypeUri { ?suppliedURI }" +
        "  OPTIONAL { ?relationshipTypeUri owl:inverseOf ?inverseRelationshipType }" +
        "}";
    ...
    while (resultSet.hasNext()) {
      ...
      try {
        Constructor<T> constructor =
            clazz.getDeclaredConstructor(Model.class, Resource.class, Resource.class);
        relationshipType = constructor.newInstance(model, relationshipTypeResource,
            inverseRelationshipTypeResource);
      } catch (InstantiationException | IllegalAccessException | IllegalArgumentException
          | InvocationTargetException | NoSuchMethodException | SecurityException e) {
        throw new CodingException("Coding error: %s thrown creating relationship: %s",
