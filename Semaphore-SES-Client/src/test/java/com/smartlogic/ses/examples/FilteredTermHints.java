...
public class FilteredTermHints {
	...
	public static void main(String[] args) throws SESException {
		...
		try (SESClient sesClient = ConfigUtil.getSESClient()) {
			...
			System.out.println(fordTerms.size() + " terms fetched in "
					+ (endCacheFetch.getTime() - startCacheFetch.getTime()) + " ms");
			...
			System.out.println(termHints.size() + " term hints fetched in "
					+ (endHintsFetch.getTime() - startHintsFetch.getTime()) + " ms");
			...
			System.out.println(filteredTermHints.size() + " filtered terms "
					+ (startFiltering.getTime() - endFiltering.getTime()) + " ms");
