...
public class FilteredTermHints {
  ...
  public static void main(String[] args) throws SESException {
    ...
    try (SESClient sesClient = ConfigUtil.getSESClient()) {
      ...
      System.out.println(fordTerms.size() +
          " terms fetched in " +
          (endCacheFetch.getTime() - startCacheFetch.getTime()) +
          " ms");

      SESFilter sesFilter = new SESFilter();
      sesFilter.setMaxResultCount(1000);
      ...
      System.out.println(termHints.size() +
          " term hints fetched in " +
          (endHintsFetch.getTime() - startHintsFetch.getTime()) +
          " ms");

      Date startFiltering = new Date();
      Map<String, TermHint> filteredTermHints = new LinkedHashMap<>();
      ...
      System.out.println(filteredTermHints.size() +
          " filtered terms " +
          (startFiltering.getTime() - endFiltering.getTime()) +
          " ms");
