...
public class TestTermsXML extends SESServerMockTestCase {
  protected static final Logger logger = LoggerFactory.getLogger(TestTermsXML.class);
  ...
  public void testMarshal() throws Exception {
    wireMockRule.stubFor(
        get(urlEqualTo("/ses?TBDB=IPSV&template=service.xml&service=term&term=Livestock+markets"))
            .willReturn(aResponse().withHeader("Content-Type", "text/xml").withBody(
                readFileToString("src/test/resources/ses/sesResponseTermLivestockMarkets.xml"))));
  ...
  public void testUnmarshall() throws Exception {
    ...
    assertEquals("Term values", "animal markets",
        term.getSynonyms().getSynonyms().get(0).getValue());
    assertEquals("Term values", "animal markets",
        term.getSynonymsList().get(0).getSynonyms().get(0).getValue());
    assertEquals("Term values", "Cattle Markets",
        term.getSynonymsList().get(0).getSynonyms().get(1).getValue());
  }
