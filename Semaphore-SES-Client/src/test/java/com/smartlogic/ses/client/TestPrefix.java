...
public class TestPrefix extends SESServerMockTestCase {
  ...
  public void testPrefixSea() throws SESException {
    ...
    wireMockRule.stubFor(
        get(urlEqualTo("/ses?TBDB=IPSV&template=service.xml&service=PREFIX&term_prefix=sea"))
    ...
    assertEquals("THS", "Public order, justice and rights",
        termHintSea.getFacets().getFacets().get(0).getName());
  ...
  public void testPrefix1() throws SESException {
    wireMockRule.stubFor(
        get(urlEqualTo("/ses?TBDB=IPSV&template=service.xml&service=PREFIX&term_prefix=chi"))
            .willReturn(aResponse().withHeader("Content-Type", "text/xml")
    ...
    assertEquals("TH1", "http://example.com/TestCM#Chickens", termHint1.getUri());
    assertEquals("TH1", 10, termHints.size()); // NOTE: this is the default we ship with.
  }

  public void testPrefix2() throws SESException {
    wireMockRule.stubFor(
        get(urlEqualTo("/ses?TBDB=IPSV&template=service.xml&service=PREFIX&term_prefix=apt"))
            .willReturn(aResponse().withHeader("Content-Type", "text/xml")
