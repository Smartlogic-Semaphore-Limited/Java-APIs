...
/**
 * To ensure JAXB compliance all objects that are persisted should be serializable and have the annotation XmlRootElement.
 * 
 * All get methods should have equivalent set methods and there should be a zero argument constructor for each.
 * @author keitha
 *
 */
...
public class TestMethodsAndInterfaces extends TestCase {
	...
	public final static Set<String> unserializableClasses = new HashSet<String>(Arrays.asList(new String[] { "SESClient", "SESClientRest", "SESFilter", "TermComparator" }));

	public final static Set<String> unXMLableClasses = new HashSet<String>(Arrays.asList(new String[] { "SESClient", "SESClientRest", "SESFilter", "TermComparator" }));
				...
				if ((method.getName().startsWith("get")) && (method.getParameterTypes().length == 0)) {
					...
					try {
						...
						if (!method.isAnnotationPresent(XmlTransient.class)) {
							fail(className + " method " + method.getName() + "has no equivalent set method: " + method.getName());
						}
					}
				}
			}
	...
	public static String[] getClassNamesInDirectory(File rootDirectory, String packageName) throws IOException{
