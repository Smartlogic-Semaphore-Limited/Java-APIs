...
public class ChoiceType implements Serializable {
  protected static final Logger logger = LoggerFactory.getLogger(ChoiceType.class);
  ...
  public ChoiceType(Element element) {
    ...
    for (int n = 0; n < nodeList.getLength(); n++) {
      ...
      if (node.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if ("VALUE_DEF".equals(childElement.getNodeName())) {
          addChoiceValue(new ChoiceValue(childElement));
        } else {
  ...
  private List<ChoiceValue> choiceValues = new ArrayList<ChoiceValue>();

  public List<ChoiceValue> getChoiceValues() {
    return choiceValues;
  }

  private void addChoiceValue(ChoiceValue choiceValue) {
    this.getChoiceValues().add(choiceValue);
  }
